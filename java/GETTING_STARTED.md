# Getting Started with Roman Numerals Addition kata in Java
...
```shell
cd Kata-RomanNumeralsAddition/java
```
...
`File` > `Open` > `Kata-RomanNumeralsAddition` > `java`
...
`File` > `Open Folder` > `Kata-RomanNumeralsAddition` > `java`
...
`File` > `Open Projects from File System...` > `Kata-RomanNumeralsAddition`
...
3. From the terminal navigate to [Kata-RomanNumeralsAddition/java]() directory
