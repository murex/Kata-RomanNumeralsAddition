# Getting Started with Roman Numerals Addition kata in C++
...
- Create a build directory: ***Kata-RomanNumeralsAddition/cpp/build***. All build-related files are generated under this directory.
...
- Generate the solution file ***Kata-RomanNumeralsAddition.sln*** for **Visual Studio 2022** on Windows,
...
Open Xcode, choose `Open a project or file`, navigate to `Kata-RomanNumeralsAddition` / `cpp` / `build` / `Kata-RomanNumeralsAddition.xcodeproj`
then click `Open`.

Once the project is opened in Xcode, select `Product` / `Scheme` / `Kata-RomanNumeralsAddition-test` to set it as the current active scheme.
